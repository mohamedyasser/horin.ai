...
const formatParameterValue = (key: string, value: unknown): string => {
    if (typeof value === 'number') {
        if (key.includes('percent') || key.includes('confidence') || key.includes('strength') || key.includes('rate')) {
...
const handleBacktest = () => {
    ...
    router.post(`/alerts/${alert.value.id}/backtest`, {
        lookback_days: 90,
...
const breadcrumbs: BreadcrumbItemType[] = [
    ...
    { title: alert.value.asset?.symbol || t('alerts.details'), href: `/alerts/${alert.value.id}` },
];
...
<template>
    <Head :title="`${alert.asset?.symbol || t('alerts.details')} - ${t('alerts.title')}`" />
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <Card>
                        ...
                        <CardContent class="space-y-4">
                            <!-- Asset Info -->
                            <div v-if="alert.asset" class="flex items-center gap-4 rounded-lg bg-muted/50 p-4">
                                <div>
                                    <p class="font-medium">{{ alert.asset.symbol }}</p>
                                    <p class="text-sm text-muted-foreground">
                                        {{ locale === 'ar' ? alert.asset.name_ar : alert.asset.name }}
                                    </p>
                                </div>
                            ...
                            <div class="grid gap-4 sm:grid-cols-2">
                                <template v-for="(value, key) in alert.parameters" :key="key">
                                    <div v-if="value !== null && value !== undefined" class="space-y-1">
                                        <p class="text-sm text-muted-foreground">{{ t(`alerts.params.${key}`, key) }}</p>
                                        <p class="font-medium">{{ formatParameterValue(key as string, value) }}</p>
                                    </div>
                                </template>
                            </div>
                            ...
                            <div class="grid gap-4 sm:grid-cols-3">
                                <div class="space-y-1">
                                    <p class="text-sm text-muted-foreground">{{ t('alerts.fields.recurring') }}</p>
                                    <p class="font-medium">{{ alert.is_recurring ? t('common.yes') : t('common.no') }}</p>
                                </div>
                                <div class="space-y-1">
                                    <p class="text-sm text-muted-foreground">{{ t('alerts.fields.cooldown') }}</p>
                                    <p class="font-medium">{{ alert.cooldown_minutes }} {{ t('common.minutes') }}</p>
                                </div>
                                <div class="space-y-1">
                                    <p class="text-sm text-muted-foreground">{{ t('alerts.fields.market_hours_only') }}</p>
                                    <p class="font-medium">{{ alert.market_hours_only ? t('common.yes') : t('common.no') }}</p>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardHeader>
                            <div class="flex items-center justify-between">
                                ...
                                >
                                    ...
                                    {{ isRunningBacktest ? t('alerts.backtest.running') : t('alerts.backtest.run') }}
                                </Button>
                            </div>
                            <CardDescription>{{ t('alerts.backtest.description') }}</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <div v-if="backtestResult" class="space-y-4">
                                ...
                                <p class="text-xs text-muted-foreground">
                                    {{ t('alerts.backtest.lookback', { days: backtestResult.lookback_days }) }}
                                    &bull;
                                    {{ t('alerts.backtest.completed_at') }}: {{ new Date(backtestResult.completed_at).toLocaleString(locale) }}
                                </p>
                            </div>
                            <div v-else class="py-8 text-center text-muted-foreground">
                                <FlaskConical class="mx-auto size-8 opacity-50" />
                                <p class="mt-2">{{ t('alerts.backtest.no_results') }}</p>
                                <p class="text-sm">{{ t('alerts.backtest.run_hint') }}</p>
                            </div>
                        </CardContent>
                    </Card>
                        ...
                        <CardHeader>
                            <div class="flex items-center justify-between">
                                ...
                                <Button as-child variant="ghost" size="sm">
                                    <Link href="/alerts-history">{{ t('common.view_all') }}</Link>
                                </Button>
                            </div>
                        </CardHeader>
                        <CardContent>
                            <div v-if="recentHistory.length > 0" class="space-y-3">
                                ...
                                >
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-medium">
                                                {{ t('alerts.triggered_at') }}: {{ new Date(history.triggered_at).toLocaleString(locale) }}
                                            </p>
                                            <p class="text-sm text-muted-foreground">
                                                {{ t('alerts.trigger_value') }}: {{ history.trigger_value }}
                                            </p>
                                        </div>
                                    </div>
                                    <Badge v-if="history.acknowledged_at" variant="outline">
                ...
                <div class="space-y-6">
                    ...
                    <Card>
                        ...
                        <CardContent class="space-y-3">
                            <!-- Snooze -->
                            <div class="space-y-2">
                                <p class="text-sm font-medium">{{ t('alerts.snooze.title') }}</p>
                            ...
                            <Button variant="outline" class="w-full" @click="handleDuplicate">
                                <Copy class="me-2 size-4" />
                                {{ t('common.duplicate') }}
                            </Button>
                        ...
                        <CardContent class="space-y-4">
                            ...
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-muted-foreground">{{ t('common.created') }}</span>
                                    <span>{{ new Date(alert.created_at).toLocaleDateString(locale) }}</span>
                                </div>
                                <div v-if="alert.last_triggered_at" class="flex justify-between">
                                    <span class="text-muted-foreground">{{ t('alerts.last_triggered') }}</span>
                                    <span>{{ new Date(alert.last_triggered_at).toLocaleDateString(locale) }}</span>
                                </div>
                                <div v-if="alert.expires_at" class="flex justify-between">
                                    <span class="text-muted-foreground">{{ t('alerts.expires') }}</span>
                                    <span>{{ new Date(alert.expires_at).toLocaleDateString(locale) }}</span>
                                </div>
                                <div v-if="alert.is_snoozed && alert.snoozed_until" class="flex justify-between">
                                    <span class="text-muted-foreground">{{ t('alerts.snoozed_until') }}</span>
                                    <span>{{ new Date(alert.snoozed_until).toLocaleString(locale) }}</span>
                                </div>
                            </div>
                        </CardContent>
                    </Card>

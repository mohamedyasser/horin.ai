...
import type { Prediction, MarketCode, PredictionHorizon, SectorCode, AssetType } from '@/types';
...
// Mock predictions data from all markets and sectors
...
const sectorCodes: SectorCode[] = ['banking', 'energy', 'telecom', 'realEstate', 'healthcare', 'industrial', 'materials', 'consumer', 'technology', 'utilities'];
const horizons: PredictionHorizon[] = ['1D', '1W', '1M', '3M'];
const assetTypes: AssetType[] = ['stock', 'etf'];
...
const selectedAssetType = ref<AssetType | null>(null);
const sortBy = ref<'gain' | 'confidence' | 'newest' | 'alphabetical' | 'market'>('gain');
...
const stats = computed(() => ({
    ...
    avgGain: (
        mockPredictions.reduce((sum, p) => sum + p.gain_percent, 0) / mockPredictions.length
    ).toFixed(1),
}));
...
// Sector display names for filter dropdown
                    ...
                    <div class="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                        ...
                        <div class="flex items-center gap-6">
                            <div class="text-center">
                                <p class="text-2xl font-bold">{{ stats.totalPredictions }}</p>
                                <p class="text-xs text-muted-foreground">{{ t('predictions.totalPredictions') }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-2xl font-bold">{{ stats.avgConfidence }}%</p>
                                <p class="text-xs text-muted-foreground">{{ t('predictions.avgConfidence') }}</p>
                            </div>
                            <div class="text-center">
                                <p class="text-2xl font-bold text-green-600 dark:text-green-400">+{{ stats.avgGain }}%</p>
                                <p class="text-xs text-muted-foreground">{{ t('predictions.avgGain') }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-4">
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger as-child>
                                    <Button variant="outline" size="sm">
                                        <Building2 class="me-1 size-4" />
                                        {{ selectedMarket ? getMarketName(selectedMarket) : t('predictions.allMarkets') }}
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger as-child>
                                    <Button variant="outline" size="sm">
                                        <Layers class="me-1 size-4" />
                                        {{ selectedSector ? getSectorName(selectedSector) : t('predictions.allSectors') }}
                                        <ChevronDown class="ms-1 size-4" />
                                    </Button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="start">
                            ...
                            <DropdownMenu>
                                <DropdownMenuTrigger as-child>
                                    <Button variant="outline" size="sm">
                                        {{ selectedHorizon || t('predictions.allHorizons') }}
                                        <ChevronDown class="ms-1 size-4" />
                                    </Button>
                                </DropdownMenuTrigger>
                                <DropdownMenuContent align="start">
                                    <DropdownMenuItem @click="selectedHorizon = null">
                                        {{ t('predictions.allHorizons') }}
                                    </DropdownMenuItem>
                                    <DropdownMenuItem
                            ...
                            <Dialog v-model:open="filterOpen">
                                ...
                                <DialogContent>
                                    <DialogHeader>
                                        <DialogTitle>{{ t('predictions.filterPredictions') }}</DialogTitle>
                                    </DialogHeader>
                                    <div class="grid gap-4 py-4">
                                        <div class="grid gap-2">
                                            <Label>{{ t('predictions.assetType') }}</Label>
                                            <DropdownMenu>
                                                <DropdownMenuTrigger as-child>
                                                    <Button variant="outline" class="w-full justify-between">
                                                        {{ selectedAssetType ? t(`predictions.${selectedAssetType}`) : t('predictions.allTypes') }}
                                                        <ChevronDown class="ms-1 size-4" />
                                                    </Button>
                                                </DropdownMenuTrigger>
                                                <DropdownMenuContent>
                                                    <DropdownMenuItem @click="selectedAssetType = null">
                                                        {{ t('predictions.allTypes') }}
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem @click="selectedAssetType = 'stock'">
                                                        {{ t('predictions.stock') }}
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem @click="selectedAssetType = 'etf'">
                                                        {{ t('predictions.etf') }}
                                                    </DropdownMenuItem>
                                                </DropdownMenuContent>
                                            </DropdownMenu>
                                        ...
                                        <div class="grid gap-2">
                                            <Label>{{ t('predictions.confidenceRange') }}</Label>
                                        ...
                                        <div class="grid gap-2">
                                            <Label>{{ t('predictions.expectedGain') }}</Label>
                                            <div class="flex gap-2">
                                        ...
                                        <div class="grid gap-2">
                                            <Label>{{ t('predictions.priceRange') }}</Label>
                                            <div class="flex gap-2">
                                                <Input type="number" placeholder="0" />
                                                <span class="flex items-center">-</span>
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuItem @click="sortBy = 'alphabetical'">
                                    {{ t('predictions.alphabetical') }}
                                </DropdownMenuItem>
                                <DropdownMenuItem @click="sortBy = 'market'">
                                    {{ t('predictions.byMarket') }}
                                </DropdownMenuItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                    </div>
                        ...
                        <Card>
                            <CardHeader class="pb-3">
                                <CardTitle class="flex items-center gap-2 text-base">
                                    ...
                                    {{ t('predictions.recentlyUpdated') }}
                                </CardTitle>
                                <p class="text-xs text-muted-foreground">
                                    {{ t('predictions.recentlyUpdatedDesc') }}
                                </p>
                            </CardHeader>
                            <CardContent class="space-y-3">
                                <Link
                                    v-for="prediction in recentlyUpdated"
                                    :key="prediction.id"
                                    :href="`/assets/${prediction.id}`"
                                    class="flex items-center justify-between hover:bg-muted/30 -mx-2 px-2 py-1 rounded transition-colors"
                                >

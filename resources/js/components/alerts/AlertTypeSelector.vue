...
const getTypeDescription = (type: AlertType): string => {
    const descriptions: Record<AlertType, { en: string; ar: string }> = {
        price: { en: 'Price targets, breakouts, zones', ar: 'أهداف السعر، الاختراقات، المناطق' },
        prediction: { en: 'AI-powered price predictions', ar: 'تنبؤات الذكاء الاصطناعي' },
        signal: { en: 'Technical indicator signals', ar: 'إشارات المؤشرات الفنية' },
        anomaly: { en: 'Unusual market behavior', ar: 'سلوك السوق غير الطبيعي' },
        pattern: { en: 'Chart pattern detection', ar: 'اكتشاف أنماط الرسم البياني' },
        recommendation: { en: 'Buy/sell recommendation changes', ar: 'تغيرات توصيات الشراء/البيع' },
    };
    return locale.value === 'ar' ? descriptions[type].ar : descriptions[type].en;
};
...
<template>
    <div class="grid grid-cols-2 gap-3 sm:grid-cols-3">
        <button
            ...
            :class="{
                'border-primary bg-primary/5': modelValue === type,
                'border-border hover:border-primary/50': modelValue !== type && !disabled,

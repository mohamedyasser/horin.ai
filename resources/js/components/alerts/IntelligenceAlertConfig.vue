...
import type { AlertParameters, AlertTriggerType } from '@/types/alerts';
import {} from 'vue';
...
const predictionTypes = [
    {
        ...
        label: { en: 'Price Direction', ar: 'اتجاه السعر' },
    },
    {
        value: 'price_target',
        label: { en: 'Price Target', ar: 'السعر المستهدف' },
    },
    {
        value: 'trend_reversal',
        label: { en: 'Trend Reversal', ar: 'انعكاس الاتجاه' },
    },
];
...
const anomalyTypes = [
    ...
    {
        value: 'volume_surge',
        label: { en: 'Volume Surge', ar: 'ارتفاع حجم التداول' },
    },
    {
        value: 'volatility_burst',
        label: { en: 'Volatility Burst', ar: 'انفجار التذبذب' },
    },
    {
        value: 'unusual_activity',
        label: { en: 'Unusual Activity', ar: 'نشاط غير عادي' },
    },
];
...
const chartPatterns = [
    {
        ...
        label: { en: 'Head & Shoulders', ar: 'الرأس والكتفين' },
    },
    { value: 'double_top', label: { en: 'Double Top', ar: 'القمة المزدوجة' } },
    {
        value: 'double_bottom',
        label: { en: 'Double Bottom', ar: 'القاع المزدوج' },
    },
        ...
        <template v-if="triggerType === 'prediction'">
            <div class="grid gap-4 sm:grid-cols-2">
                <div class="grid gap-2">
                    ...
                    <Select
                        :model-value="
                            parameters.prediction_type || 'price_direction'
                        "
                        @update:model-value="
                            updateParameter('prediction_type', $event)
                        "
                    >
                        <SelectTrigger>
                            <SelectValue />
                ...
                <div class="grid gap-2">
                    ...
                    <Select
                        :model-value="parameters.horizon || '1day'"
                        @update:model-value="updateParameter('horizon', $event)"
                    >
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
            ...
            <div class="grid gap-2">
                <Label
                    ...
                    {{
                        ((parameters.min_confidence || 0.75) * 100).toFixed(0)
                    }}%</Label
                >
                <Slider
                    ...
                    :step="5"
                    @update:model-value="
                        updateParameter('min_confidence', $event[0] / 100)
                    "
                />
            </div>
        </template>
            ...
            <div class="grid gap-4">
                        ...
                        <button
                            ...
                            :class="{
                                'border-foreground bg-muted text-foreground':
                                    isSelected('indicators', indicator.value),
                                'hover:border-foreground/50': !isSelected(
                                    'indicators',
                                    indicator.value,
                                ),
                            }"
                            @click="
                                toggleArrayItem('indicators', indicator.value)
                            "
                        >
                            {{ indicator.label }}
                        </button>
                ...
                <div class="grid gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="{
                                'border-foreground bg-muted text-foreground':
                                    isSelected('signal_types', signal.value),
                                'hover:border-foreground/50': !isSelected(
                                    'signal_types',
                                    signal.value,
                                ),
                            ...
                            @click="
                                toggleArrayItem('signal_types', signal.value)
                            "
                        >
                            {{
                                locale === 'ar'
                ...
                <div class="grid gap-2">
                    <Label
                        ...
                        {{
                            ((parameters.min_strength || 0.7) * 100).toFixed(0)
                        }}%</Label
                    >
                    <Slider
                        ...
                        :step="5"
                        @update:model-value="
                            updateParameter('min_strength', $event[0] / 100)
                        "
                    />
                </div>
            </div>
            ...
            <div class="grid gap-4">
                <div class="grid gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="{
                                'border-foreground bg-muted text-foreground':
                                    isSelected('anomaly_types', type.value),
                                'hover:border-foreground/50': !isSelected(
                                    'anomaly_types',
                                    type.value,
                                ),
                            ...
                            @click="
                                toggleArrayItem('anomaly_types', type.value)
                            "
                        >
                            {{
                ...
                <div class="grid gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="{
                                'border-foreground bg-muted text-foreground':
                                    isSelected('severity', level.value),
                                'hover:border-foreground/50': !isSelected(
                                    'severity',
                                    level.value,
                                ),
                ...
                <div class="grid gap-2">
                    <Label
                        >{{ t('alerts.fields.min_confidence') }}:
                        {{
                            ((parameters.min_confidence || 0.8) * 100).toFixed(
                                0,
                            )
                        }}%</Label
                    >
                    <Slider
                        :model-value="[
                            (parameters.min_confidence || 0.8) * 100,
                        ]"
                        :min="50"
                        :max="95"
                        :step="5"
        ...
        <template v-else-if="triggerType === 'pattern'">
            <div class="grid gap-4">
                <div class="grid gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="{
                                'border-foreground bg-muted text-foreground':
                                    isSelected('patterns', pattern.value),
                                'hover:border-foreground/50': !isSelected(
                                    'patterns',
                                    pattern.value,
                                ),
                ...
                <div class="grid gap-2">
                    ...
                    <Select
                        :model-value="parameters.pattern_status || 'confirmed'"
                        @update:model-value="
                            updateParameter('pattern_status', $event)
                        "
                    >
                        <SelectTrigger>
                            <SelectValue />
                        ...
                        <SelectContent>
                            <SelectItem value="forming">{{
                                t('alerts.pattern_status.forming')
                            }}</SelectItem>
                            <SelectItem value="confirmed">{{
                                t('alerts.pattern_status.confirmed')
                            }}</SelectItem>
                            <SelectItem value="completed">{{
                                t('alerts.pattern_status.completed')
                            }}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>
                <div class="grid gap-2">
                    <Label
                        >{{ t('alerts.fields.min_confidence') }}:
                        {{
                            ((parameters.min_confidence || 0.7) * 100).toFixed(
                                0,
                            )
                        }}%</Label
                    >
                    <Slider
                        :model-value="[
                            (parameters.min_confidence || 0.7) * 100,
                        ]"
                        :min="50"
                        :max="95"
                        :step="5"
        ...
        <template v-else-if="triggerType === 'recommendation'">
            <div class="grid gap-4">
                <div class="grid gap-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        <button
                            ...
                            :class="{
                                'border-foreground bg-muted text-foreground':
                                    isSelected('recommendations', rec.value),
                                'hover:border-foreground/50': !isSelected(
                                    'recommendations',
                                    rec.value,
                                ),
                            ...
                            @click="
                                toggleArrayItem('recommendations', rec.value)
                            "
                        >
                ...
                <div class="grid gap-2">
                    ...
                    <Select
                        :model-value="parameters.trigger_on || 'change'"
                        @update:model-value="
                            updateParameter('trigger_on', $event)
                        "
                    >
                        <SelectTrigger>
                            <SelectValue />
                        ...
                        <SelectContent>
                            <SelectItem value="change">{{
                                t('alerts.recommendation_trigger.change')
                            }}</SelectItem>
                            <SelectItem value="upgrade">{{
                                t('alerts.recommendation_trigger.upgrade')
                            }}</SelectItem>
                            <SelectItem value="downgrade">{{
                                t('alerts.recommendation_trigger.downgrade')
                            }}</SelectItem>
                        </SelectContent>
                    </Select>
                </div>

...
const conditionTypes = [
    {
        ...
        label: { en: 'Price Threshold', ar: 'حد السعر' },
        icon: '💰',
    },
    {
        value: 'signal',
        label: { en: 'Technical Signal', ar: 'إشارة فنية' },
        icon: '📉',
    },
    { value: 'anomaly', label: { en: 'Anomaly', ar: 'شذوذ' }, icon: '⚠️' },
    {
        value: 'pattern',
        label: { en: 'Chart Pattern', ar: 'نمط الرسم' },
        icon: '📊',
    },
    {
        value: 'recommendation',
        label: { en: 'Recommendation', ar: 'توصية' },
        icon: '💡',
    },
    {
        value: 'prediction',
        label: { en: 'AI Prediction', ar: 'توقع ذكي' },
        icon: '🔮',
    },
];
...
const logicOptions = [
    {
        value: 'and',
        label: { en: 'ALL conditions (AND)', ar: 'جميع الشروط (و)' },
    },
...
const removeCondition = (id: string) => {
    emit(
        'update:conditions',
        props.conditions.filter((c) => c.id !== id),
    );
};
    ...
    <div class="space-y-4">
        ...
        <div class="space-y-2">
            ...
            <div class="flex gap-2">
                <button
                    ...
                    :class="{
                        'border-foreground bg-muted': logic === opt.value,
        ...
        <div class="space-y-3">
            <div class="flex items-center justify-between">
                ...
                <Badge variant="outline"> {{ conditions.length }}/5 </Badge>
            </div>
                ...
                >
                    <CardContent class="p-4">
                        <div class="flex items-start gap-3">
                            <div
                                class="flex size-8 shrink-0 cursor-grab items-center justify-center rounded-md bg-muted"
                            >
                                <GripVertical
                            ...
                            <div class="min-w-0 flex-1 space-y-3">
                                ...
                                <div class="grid gap-3 sm:grid-cols-2">
                                    ...
                                    <template
                                        v-if="
                                            condition.type === 'price_threshold'
                                        "
                                    >
                                        <div class="grid gap-1">
                                            <Label class="text-xs">{{
                                                t('alerts.fields.threshold')
                                            }}</Label>
                                            <Input
                                                :model-value="
                                                    condition.parameters
                                    ...
                                    <!-- Signal -->
                                    <template
                                        v-else-if="condition.type === 'signal'"
                                    >
                                    ...
                                    <!-- Anomaly -->
                                    <template
                                        v-else-if="condition.type === 'anomaly'"
                                    >
                                        <div class="col-span-2 grid gap-1">
                                            <Label class="text-xs">{{
                                                t('alerts.fields.min_score')
                                            }}</Label>
                                            <Input
                                                :model-value="
                                                    condition.parameters
                                    ...
                                    >
                                        <div class="col-span-2 grid gap-1">
                                            <Label class="text-xs">{{
                                                t(
                                                    'alerts.fields.min_confidence',
                                                )
                                            }}</Label>
                                            <Input
                                                :model-value="
                                                    condition.parameters
                                                        .min_confidence as number
                                                "
                                                type="number"
                                                step="0.05"
                                                min="0"
                                    ...
                                    >
                                        <div class="grid gap-1">
                                            <Label class="text-xs">{{
                                                t(
                                                    'alerts.fields.prediction_direction',
                                                )
                                            }}</Label>
                                            <Select
                                                :model-value="
                                            ...
                                            >
                                                ...
                                                <SelectContent>
                                                    <SelectItem value="up">{{
                                                        t(
                                                            'alerts.direction.bullish',
                                                        )
                                                    }}</SelectItem>
                                                    <SelectItem value="down">{{
                                                        t(
                                                            'alerts.direction.bearish',
                                                        )
                                                    }}</SelectItem>
                                                    <SelectItem value="both">{{
                                                        t(
                                                            'alerts.direction.both',
                                                        )
                                                    }}</SelectItem>
                                                </SelectContent>
                                            </Select>
